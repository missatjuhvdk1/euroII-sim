...
# Initialize step sizes in session state
...
with st.expander("Gemeten Concentratie en Stapgroottes per Element", expanded=True):
    ...
    for idx, row in df.iterrows():
        ...
        with cols[idx % 2]:
            if element != 'Geen':  # Skip elements like 'Geen' for Demiwater
                ...
                # Optionally, display a disabled step size input for consistency
                col_conc, col_step = st.columns([2, 1])
                with col_conc:
                    st.number_input(
                        f"{element} ({eenheid}) voor {grondstof}",
...
if st.button("Bereken Geoptimaliseerde Toevoegingen", help="Klik om de optimalisatie te starten op basis van de ingevoerde concentraties."):
    ...
    if status == "Succes":
        ...
        st.dataframe(optimized_resultaten.style.applymap(highlight_specs, subset=['Geoptimaliseerde_Binnen_Specs']).format({
            'Geoptimaliseerde_Toevoegen_Grondstof': '{:.0f}',
            'Gemeten_Concentratie': '{:.2f}',
            'Geoptimaliseerde_Concentratie': '{:.2f}'
        }))
