...
# Titel van de app
st.title("Correctie Recept Calculator")
...
with st.expander("Algemene Gegevens", expanded=True):
    ...
    with col2:
        dichtheid = st.number_input("Dichtheid (kg/l)", value=recepten[recept]["Dichtheid"], step=0.01, min_value=0.0, help="De dichtheid van het product in kg per liter.")
...
def bereken_actuele_hoeveelheid_grondstof(row):
    if row['Ratio_Element'] == 0:
        return 0.0  # Handle zero ratio case (e.g., Demiwater)
    ...
    for idx, (grondstof, value) in enumerate(st.session_state.initiele_schattingen.items()):
        with cols[idx % 3]:
            st.session_state.initiele_schattingen[grondstof] = st.number_input(
                ...
                step=1.0, 
                min_value=0.0, 
                key=f"init_{recept}_{grondstof}",  # Unique key per recept
