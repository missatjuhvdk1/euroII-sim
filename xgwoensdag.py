...
with st.expander("Algemene Gegevens", expanded=True):
    col1, col2 = st.columns(2)  # Only two columns needed now
    with col1:
...
# State voor initiële schattingen en stapgroottes
if 'initiele_schattingen' not in st.session_state or st.session_state.get('current_recept') != recept:
    ...
    st.session_state.stapgroottes = {row['Grondstof']: 0 for _, row in df.iterrows()}  # Initialize step sizes
...
with st.expander("Initiële Schattingen en Stapgroottes voor Optimalisatie (kg)", expanded=True):
    ...
    cols = st.columns(2)  # Use 2 columns to fit both inputs
    for idx, grondstof in enumerate(df['Grondstof']):
        with cols[idx % 2]:
            col_input, col_step = st.columns([2, 1])  # Sub-columns for guess and step size
            with col_input:
            ...
            with col_step:
                step_value = st.number_input(
                    "Stap (kg)", 
                    value=st.session_state.stapgroottes[grondstof], 
                    step=1, 
                    min_value=0, 
                    key=f"step_{recept}_{grondstof}",
